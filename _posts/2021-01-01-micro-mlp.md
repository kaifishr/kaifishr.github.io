...
Next, we can lookt at the implementation of the backpropagation algorithm which can be divided into three separate modules. The `backpropagation()` method executes the layer-wise gradient computation starting at the output layer. 
...
The initial delta is computed using the `comp_delta_init()` function. This function is only called once.
...
As we have seen during the derivation of the backpropagation algorithm, the computation of all other deltas is the same for all layers which allows us to use a single `comp_delta()` function for all remaining layers.
...
The gradient descent part of the algorithm is actually the most simple part of the algorithm. The `gradient_descent()` method updates the trainable parameters connecting adjacent layers.
...
For this pattern we probably need to increase the model's capacity to allow for a better representation. For a cleaner visualization of both prediction landscapes below, I removed the validation set data points.
