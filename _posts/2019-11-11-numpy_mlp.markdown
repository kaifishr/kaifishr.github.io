...
`network_config = (n_input, 128, 128, n_classes)`

represents a four-layered network (`network_depth = len(network_config)`), that consists of two hidden layers of 128 neurons each. Next, we initialize the network's trainable parameters, that is its weights `W` and biases `b`:
...
$$w_{ij} \sim \mathcal{N} (0, \sqrt{\frac{2}{n_l}})$$
...
def feedforward(X):
    ...
    a[-1] = sigma(z[-1])
```
(for the sake of better readability I skipped the `self` terms in the feedforward function above)

<!--
### Dropout

A very simple but effective form of regularization is [dropout][Dropout]. The dropout process randomly drops a certain number of activations during the feedforward pass.

```python
def dropout(x, dropout_rate):
    return x * (np.random.random(size=x.shape) > dropout_rate)
```

Dropout helps the network to avoid overfitting by reducing correlations between neurons and by that improves the network's generalization capabilities. That is, the networks capabilities to perform well on previously unseens examples. From an ensemble point of perspective, dropout can be considered as a method that allows to train a large amount of networks at the same time in parallel.
-->
...
<!--
A more advanced and widely used loss function that is commonly used in classification tasks is the so called cross entropy. This loss function is applied to the network's output neurons which have been transformed by a softmax activation function which is defined as follows: 

$$a_i^{(L)} = \frac{\mathcal{e}^{z_i^{(L)}}}{\sum_{k=1}^{n^{(L)}}\mathcal{e}^{z_k^{(L)}}}$$

The softmax operation squeezes the networks real-valued output, i.e. its pre-activations, in a range between 0 and 1. Addidtionally the sum of the output neurons after the softmax operation sums up to 1 and thus the single activations can be interpreted as probabilities or the networks confidence about the classification.

The error then is computed by inserting the newly computed probabilities into the cross-entropy loss function which is defined as follows

$$C(y,a) = - \sum_{i=1}^{n^{(L)}} y_i \log (a_i^{(L)})$$

In Python, we can do the following to implement the softmax cross-entropy loss:

```python
def softmax(x):
    x_ = expit(x)   # add numerical stability
    return x_ / np.sum(x_, axis=1, keepdims=True)

def cross_entropy(y_true, y_pred):
    return np.log(y_true) * y_pred
...
<!-- NOTE Check += in aboves implementation -->
...
It is interesting to see, that the neural network learns to extracts salient features by learning certain filters that can amplify and suppress characteristics of an input. The visualization of the weights also shows, that some neurons may not contribute a great deal to the classification as their associated filters consist almost exlusively of noise.

## Discussion

This project is a very basic implementation of a fully connected neural network with many possibilities for improvement such as L2 regularization, dropout, batch normalization, softmax cross-entropy loss, to name but a few. The results for the three-layered fully connected neural network shows good performance for the MNIST and Fashion-MNIST dataset. For both datasets, loss and accuracy of the training dataset indicate tendencies towards overfitting. The weight visualization gives a rough clue which features of the input images are of particular importance for a network to map its input to the correct output.
